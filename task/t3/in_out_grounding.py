...
def format_user_document(user: dict[str, Any]) -> str:
    ...
    context_str = ""
    context_str += f"User:\n"
    context_str += f"id: {user["id"]}\n"
    context_str += f"About user: {user["about_me"]}"
    context_str += "\n"
    return context_str
...
class InputGrounder:
    ...
    async def initialize_vectorstore(self, batch_size: int = 50):
        ...
        docs = [Document(id=user.get('id'), page_content=format_user_document(user)) for user in users]
        ...
        print("Vecrorstore was initialized")
    ...
    async def _update_vectorstore(self):
        ...
        new_users_ids = users_ids_set - vectorstore_ids_set
        ...
        new_documents = [Document(id=user_id, page_content=format_user_document(users_dict[user_id])) for user_id in new_users_ids]
        # 10. Id new documents are present then save them to vectorstore
        if new_documents:
            if len(new_documents) > 99:
                batches = [new_documents[i:i + 99] for i in range(0, len(new_documents), 99)]
